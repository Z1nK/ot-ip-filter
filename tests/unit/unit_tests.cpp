...
TEST(LogReaderTest, ReadParsesValidTsvAndSkipsEmptyLines) {
  std::istringstream input("1.2.3.4\t10\t20\n"
                           "\n"
                           "46.70.1.2\t30\t40\n");
...
TEST(LogReaderTest, ReadThrowsWhenLineStartsWithTabAndHasNoIp) {
...
TEST(LogReaderTest, ReadAcceptsOnlyIpWithoutTabAtEndOfInput) {
  std::istringstream input("46.70.1.2");

  const std::vector<uint32_t> ips = LogReader::read(input);

  ASSERT_EQ(ips.size(), 1u);
  EXPECT_EQ(ips[0], 0x2E460102u);
}

TEST(LogReaderTest, ReadThrowsOnInvalidIpAddress) {
  std::istringstream input("300.2.3.4\t10\t20\n");

  EXPECT_THROW(LogReader::read(input), std::invalid_argument);
}
